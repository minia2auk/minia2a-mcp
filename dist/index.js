...
function agentId() {
    if (_agentId)
        return _agentId;
    if (process.env.MINIA2A_AGENT_ID)
        return (_agentId = process.env.MINIA2A_AGENT_ID);
    const candidates = [
...
async function fetchServices() {
    ...
    if (!res.ok)
        throw new Error(`minia2a API returned ${res.status}`);
    const json = await res.json();
    const data = (json.services || json);
...
async function registerAgent(name, wallet, signature) {
    let address = wallet;
    let sig = signature;
...
function makePayingFetch(privateKey) {
...
// ── Tool: list_services ──────────────────────────────────────────────
server.tool("minia2a_list_services", "List available x402 services on minia2a.uk — the micropayment marketplace for AI agents. Returns service name, description, price, and endpoint for each service. Use this to discover what capabilities are available before calling them.", {
    category: z
        .string()
        .optional()
...
}, { readOnlyHint: true }, async ({ category, search, limit }) => {
...
// ── Tool: get_service ────────────────────────────────────────────────
server.tool("minia2a_get_service", "Get detailed information about a specific x402 service on minia2a.uk, including its price, endpoint URL, input schema, and usage instructions. Use this before calling a service to understand what it needs.", {
    serviceId: z.string().describe("The service ID or name to get details for"),
}, { readOnlyHint: true }, async ({ serviceId }) => {
...
// ── Tool: get_stats ──────────────────────────────────────────────────
server.tool("minia2a_get_stats", "Get current platform statistics for minia2a.uk — total services, registered agents, transaction volume, uptime, and more. Useful for understanding the marketplace's scale and health.", {}, { readOnlyHint: true }, async () => {
...
}, { destructiveHint: true }, async ({ name, wallet, signature }) => {
    try {
        const { ok, data, wallet: address, privateKey } = await registerAgent(name, wallet, signature);
        if (ok) {
            return {
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            ...(privateKey
                                ? {
                                    privateKey,
                                    store_this: "Store this private key securely — minia2a never holds it. It signs your future calls.",
                                }
                                : {}),
                            freeTrial: "5 free trial calls per signed wallet across all services (no registration needed).",
...
server.tool("minia2a_call_service", "Call an x402 service on minia2a.uk. Three access paths: (1) omit everything for the paid 402 path (sign a wallet for 5 free trial calls — no registration); (2) pass privateKey (or set MINIA2A_PRIVATE_KEY) for your wallet's own 5 trials — the key never leaves this process, it only signs the per-call trial message; (3) when both are exhausted the endpoint returns HTTP 402 with a machine-readable accepts[] array — pay in USDC and retry with a PAYMENT-SIGNATURE header (x402 V2). Set autoPay:true together with privateKey to have a 402 paid automatically in USDC on Base and the call retried — the wallet must hold USDC or the call still returns payment_required (never charges silently). Note that wallet= on its own does NOT reach the wallet bucket; the signature is what does.", {
...
}, { destructiveHint: true }, async ({ serviceId, params, wallet, privateKey, autoPay }) => {
    const services = await fetchServices();
    ...
    const service = services.find((s) => s.id.toLowerCase() === want) ??
    ...
    if (key) {
        try {
            ...
            headers["X-Wallet-Signature"] = await signer.signMessage(`minia2a trial:${trialSigner}:${service.id}:${ts}`);
    ...
    if (walletParam)
        url.searchParams.set("wallet", walletParam);
...
function readChallenge(resp, body) {
...
server.tool("minia2a_check_endpoint", "Validate any x402 endpoint for autonomous-agent (Claude Code auto-mode) readiness. Checks 9 signals: HTTP reachability, JSON content-type, payment challenge (canonical PAYMENT-REQUIRED header or legacy x-402-* headers), payment amount, network, recipient, trial info, registration path, and /api/agent-ready handshake. Returns a score (0-100%) with per-check PASS/FAIL detail. Use this before calling a paid endpoint to verify it works with autonomous agents.", {
    ...
    const push = (signal, weight, pass, detail, credited) => checks.push({ signal, weight, credited: credited ?? (pass === true ? weight : 0), pass, detail });
    ...
    try {
        ...
        push("HTTP 200/402", 15, httpOk, `HTTP ${probeResp.status}${probeResp.ok ? " OK" : " (reachable, payment required)"}` +
            (httpOk && !probeResp.ok ? " — half credit: reachable and speaking x402, but not answering the call" : ""), httpOk ? (probeResp.ok ? 15 : 7.5) : 0);
        // 2. JSON Content-Type
        ...
        const acc = (challenge?.accepts && challenge.accepts[0]) || {};
        const hasChallenge = probeResp.status === 402 && (!!acc.amount || !!acc.network || !!legacyAmount);
        push("Payment challenge", 15, hasChallenge, hasChallenge
        ...
        push("Payment amount", 10, amountOk, amountOk
        ...
        push("Payment network", 10, netOk, netOk
        ...
        push("Payment recipient", 10, payToOk, payToOk ? String(payTo) : payTo ? `too short to be an address: ${payTo}` : "no payTo in challenge");
        ...
        if (body !== null) {
            ...
            const declared = typeof body.trialExhausted === "boolean" ||
                /\btrial\b/i.test([...(Array.isArray(body.nextSteps) ? body.nextSteps : []), body.message || ""].join(" "));
            const hasTrial = counted || declared;
            push("Trial info", 15, hasTrial, counted
            ...
            push("Registration path", 15, hasReg, regPath
    ...
    try {
        ...
        if (arResp.ok) {
            ...
            const nested = arBody.payment && arBody.payment.chain && arBody.payment.token &&
                arBody.registration && arBody.registration.endpoint;
            const flat = typeof arBody.status === "string" &&
            ...
            push("Agent-Ready endpoint", 15, ready, ready
                ? flat
